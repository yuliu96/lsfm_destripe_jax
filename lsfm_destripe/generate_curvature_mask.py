...
def compute_curvature(contour, window_size):
    m, n = contour.shape
    m = m - window_size + 1
    neighborhood = skimage.util.view_as_windows(contour, (window_size, 1)).reshape(
        m, n, -1
    )
    x_neighborhood = neighborhood[:, 1, :]
    y_neighborhood = neighborhood[:, 0, :]
    tangent_direction_original = np.arctan2(
        np.gradient(y_neighborhood, axis=-1), np.gradient(x_neighborhood, axis=-1)
    )
    tangent_direction_original = tangent_direction_original[
        :, window_size // 2 : window_size // 2 + 1
    ].repeat(window_size, axis=-1)
    translated_x = (
        x_neighborhood - contour[window_size // 2 : -window_size // 2 + 1, 1][:, None]
    )
    translated_y = (
        y_neighborhood - contour[window_size // 2 : -window_size // 2 + 1, 0][:, None]
    )
    rotated_x = translated_x * np.cos(
        -tangent_direction_original
    ) - translated_y * np.sin(-tangent_direction_original)
...
def plot_edges_with_curvature(mask, min_contour_length, window_size_ratio):
    edge_pixels, curvature_values = compute_curvature_profile(
        mask, min_contour_length, window_size_ratio
    )
    mask = np.zeros_like(mask)
    mask[
        edge_pixels[np.abs(curvature_values) > 0.5, 0].astype(int),
        edge_pixels[np.abs(curvature_values) > 0.5, 1].astype(int),
    ] = 1
    return mask
...
def generate_mask(mask, min_contour_length=20, window_size_ratio=5):
    ...
    edges = cv2.Canny(
        image=img_blur.astype(np.uint8), threshold1=ot, threshold2=ot * 2
